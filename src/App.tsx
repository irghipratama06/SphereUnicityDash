...
import { claimDeposit, getTreasury, loadLeaderboard, loadPlayer, submitRun, type LeaderboardRow, type Player } from './api';
...
export default function App() {
  ...
  const deposit = async () => {
    ...
    try {
      const result = await payDeposit(session, treasury);
      const transferId = (result as { transferId?: string }).transferId;
      // The backend verifies the incoming transfer against the treasury wallet before crediting attempts.
      const credited = await claimDeposit({ wallet, transferId });
      setPlayer((old) => ({ ...old, attempts: credited.attempts }));
      setMessage(`Deposit verified. +${credited.added} attempts.`);
      setBalance(await getBalance(session));
      await refreshLeaderboard();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Deposit failed or was not verified.');
    } finally {
      setBusy(false);
    }
  };
  ...
  useEffect(() => {
    let alive = true;
    getTreasury().then((data) => alive && setTreasury(data.address)).catch(() => setMessage('Backend is not configured yet. See README.'));
    refreshLeaderboard();
    return () => { alive = false; };
  }, [refreshLeaderboard]);
  ...
  return (
    <main className="app-shell">
      ...
      <section className="action-row">
        <button className="deposit-button" onClick={deposit} disabled={!session || busy || !treasury}>
      ...
      <section className="leaderboard-section">
        <div className="section-heading">
          <div>
            <div className="eyebrow">LIVE DATABASE</div>
            <h2>LEADERBOARD</h2>
          </div>
